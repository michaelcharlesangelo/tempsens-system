...
export default function AdminPage() {
  ...
  useEffect(() => {
    loadStations(); loadAccounts(); loadSales(); loadCategories();
  }, []);
  ...
  // Optimistic local swap first (instant feel), then confirm with the
  // server in the background - fixes the noticeable lag from re-fetching
      ...
      {adminTab === "sales" && (
        <div className="card">
          ...
          <div className="grid">
            ...
            <div className="field"><label>Email (optional for now)</label><input type="text" value={newSalesEmail} onChange={(e) => setNewSalesEmail(e.target.value)} /></div>
            <div className="field"><label>Password (optional for now)</label><input type="password" value={newSalesPassword} onChange={(e) => setNewSalesPassword(e.target.value)} /></div>
          </div>
          <button className="btn secondary" onClick={addSalesPerson} disabled={!newSalesName.trim()}>Add</button>
          {salesPeople.length > 0 && (
            <table className="data-table" style={{ marginTop: 14 }}>
              <thead><tr><th>Name</th><th>Email</th></tr></thead>
              <tbody>{salesPeople.map((p) => <tr key={p.id}><td>{p.name}</td><td>{p.email || "-"}</td></tr>)}</tbody>
            </table>
          )}
        </div>
      )}
