...
export default function AdminPage() {
  ...
  async function loadCatalog() {
    setCatalogItems((await (await fetch("/api/item-catalog", { cache: "no-store" })).json()).items ?? []);
  }
  ...
  return (
    <>
      ...
      {adminTab === "items" && (
        <div className="card">
          <h2>Items (component-level, from BOM entries)</h2>
          <p className="subtle">Every item code + description used so far across all BOMs, sorted alphabetically. Auto-fills when the same code is typed again.</p>
          {catalogItems.length === 0 ? <p className="subtle">None yet.</p> : (
            <table className="data-table">
              <thead><tr><th>Item Code</th><th>Description</th></tr></thead>
              <tbody>
                {catalogItems.map((i) => <tr key={i.item_no}><td>{i.item_no}</td><td>{i.description}</td></tr>)}
              </tbody>
            </table>
          )}
        </div>
