...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // costingDone still applies after completion (that's when costing
  // happens) - only the production-side fields lock once finished.
  const productionLocked = jobOrder?.status === "completed";
