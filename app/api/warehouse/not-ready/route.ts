...
interface JoSummary { id: string; jo_number: string; customer_name: string; so_no: string; acknowledged_at: string | null; }
...
export async function GET() {
  ...
  const { data: joRowsRaw } = await admin
    .from("job_orders")
    .select("id, jo_number, customer_name, so_no, acknowledged_at")
