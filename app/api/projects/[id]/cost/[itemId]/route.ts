...
export async function PATCH(req: NextRequest, { params }: { params: { itemId: string } }) {
  ...
  if (typeof body.poCode === "string") updates.po_code = body.poCode.trim().toUpperCase();
  ...
  if (body.qty !== undefined || body.unitPrice !== undefined) {
    const { data: current } = await admin.from("project_cost_items").select("qty, unit_price").eq("id", params.itemId).maybeSingle();
  ...
  const { data, error } = await admin.from("project_cost_items").update(updates).eq("id", params.itemId).select().single();
...
export async function DELETE(req: NextRequest, { params }: { params: { itemId: string } }) {
  const admin = getSupabaseAdminClient();
  const { error } = await admin.from("project_cost_items").delete().eq("id", params.itemId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
