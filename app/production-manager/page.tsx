...
export default function ProductionManagerPage() {
  ...
  const [availableTemplate, setAvailableTemplate] = useState<{ bom_snapshot: { itemNo: string; description: string; qty: number; unit: string }[]; source_jo_number: string } | null>(null);
  ...
  async function openBomModal(jo: JobOrder) {
    setBomTarget(jo);
    setRows(Array.from({ length: ROW_COUNT }, () => ({ ...BLANK_ROW })));
    setAvailableTemplate(null);
    if (jo.item_no) {
      const res = await fetch(`/api/bom-templates/${encodeURIComponent(jo.item_no)}`, { cache: "no-store" });
      const data = await res.json();
      if (data.template) setAvailableTemplate(data.template);
    }
  }

  function useTemplate() {
    if (!availableTemplate) return;
    const filled = availableTemplate.bom_snapshot.map((r) => ({
      itemNo: r.itemNo, description: r.description, qty: String(r.qty), unit: r.unit, materialReady: true,
    }));
    const padded = [...filled];
    while (padded.length < ROW_COUNT) padded.push({ ...BLANK_ROW });
    setRows(padded.slice(0, Math.max(ROW_COUNT, filled.length)));
  }
    ...
    <>
      ...
      {bomTarget && (
        <div className="card">
          ...
          {availableTemplate && (
            <div className="warn" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <span>Found a previous BOM for this item (used on {availableTemplate.source_jo_number}).</span>
              <button className="btn secondary" onClick={useTemplate}>Use previous BOM</button>
            </div>
          )}
