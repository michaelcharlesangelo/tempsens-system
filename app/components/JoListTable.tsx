      ...
      <table className="data-table">
        ...
        <tbody>
          {items.map((jo) => {
            ...
            const rejectedByEntry = [...(jo.history ?? [])].reverse().find((h) => h.status === "rejected");
            return (
              <Fragment key={jo.id}>
                <tr>
                  ...
                  {showProgress && (
                    <td>
                      ...
                      {jo.status === "rejected" && rejectedByEntry && (
                        <div className="subtle" style={{ fontSize: "0.7rem", marginTop: 3 }}>
                          Rejected by {rejectedByEntry.changed_by} ({fmtDateTime(rejectedByEntry.changed_at)})
                        </div>
                      )}
                    </td>
                  )}
