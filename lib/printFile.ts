...
export function printFileUrl(url: string, isPdf: boolean) {
  ...
  if (isPdf) {
    w.document.write(
      `<html><head>${meta}<title>Print</title>${pageStyle}</head><body style="margin:0">` +
    ...
    w.document.write(
      `<html><head>${meta}<title>Print</title>${pageStyle}</head><body style="margin:0;text-align:center">` +
